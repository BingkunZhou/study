        ...
        */
        ...
        if(char_Opcodes[k] == check_Opcodes_M[0]) {
            ...
            if (First_Type[k] == check_type_R[0]) {
                ...
                //MOV REG 0 VAL 3
                if(Second_Type[k] == check_type_V[0]) {
                    //0
                    int index = First_Value_int[k];
                    //{3}
                    Reg_val[index] = Second_Value_int[k];
                }
                //more steps ...
            }
            //MOV STK
            if (First_Type[k] == check_type_S[0]) {
                int index = ((int)(First_Value_A[0]))-65;
                Stk_val[index] = Second_Value_int[k];
            }


        }
        //REF STK
        else if(First_Type[k] == check_Opcodes_F[0]) {
            //REF STK1 STK2
            if (Second_Type[k] == check_type_R[0]) {
                int index1 = ((int)(First_Value_A[0]))-65;// 1 B
                int index2 = ((int)(Second_Value_A[0]))-65;//0 A
                Stk_val[index1] =  Stk_val[index2];
            }
        }
